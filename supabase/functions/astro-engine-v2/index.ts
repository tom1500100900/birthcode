...
function buildV2Result(profile: ProfileRow) {
  const seedBase = [
    profile.id,
    profile.birth_date,
    profile.birth_time,
...
serve(async (req: Request) => {
  try {
    ...
    console.log('[astro][edge-v2] request', { profileId, hasDeviceHeader: Boolean(deviceHeader), hasAuthHeader: Boolean(authHeader) });
    ...
    if (profileError) {
      console.error('[astro][edge-v2] profile query failed', profileError);
    ...
    if (cachedError) {
      console.error('[astro][edge-v2] cache query failed', cachedError);
    ...
    if (cached) {
      console.log('[astro][edge-v2] cache hit', { profileId });
      return new Response(
        JSON.stringify({
          source: 'cache',
          engine_version: ENGINE_VERSION,
    ...
    const computed = buildV2Result(row);
    const insertPayload = {
      profile_id: row.id,
    ...
    if (insertError) {
      console.error('[astro][edge-v2] insert failed', insertError);
    ...
    console.log('[astro][edge-v2] computed', { profileId });
    return new Response(
      JSON.stringify({
        source: 'computed',
        engine_version: ENGINE_VERSION,
    ...
    console.error('[astro][edge-v2] unhandled error', error);
